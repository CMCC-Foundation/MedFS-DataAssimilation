...
WRITE(IOUNLOG,*) ' BEGIN OF PREPINTERP_INIT3' ; CALL FLUSH(IOUNLOG)
...
WRITE(IOUNLOG,*) ' END OF PREPINTERP_INIT3' ; CALL FLUSH(IOUNLOG)
WRITE(IOUNLOG,*)  ; CALL FLUSH(IOUNLOG)

LLINIT = .TRUE.

END SUBROUTINE PREPINTERP_INIT3
...
SUBROUTINE PREPINTERP3(LON,LAT,IBS,JBS,PBS)
...
!JENNY CALCOLA IO J0 COME IN MFS
 DO JJ=JST,JEN
    DO JI=IST,IEN

    ZD=INTLON(JI,JJ)-LON
    ZD_LAT=INTLAT(JI,JJ)-LAT

    IF (ZD .GT. 0  .AND. ZD_LAT .GT. 0 ) GOTO 18
  ENDDO
 ENDDO
 18 CONTINUE
    I0=JI
    J0=JJ
WRITE(IOUNLOG,*) ' END OF IO'
   IBS(1) = JI-1 
   JBS(1) = JJ-1
   IBS(2) = JI
   JBS(2) = JJ-1 
  ...
  DO KI=1,4
     ZDA(KI)=_MYDIST_(LON,LAT,INTLON(IBS(KI),JBS(KI)),INTLAT(IBS(KI),JBS(KI)))
  ENDDO

DO K=1,4
 PBS(K) = 1._R8 / MAX(1.E-12_R8,ZDA(K))
ENDDO
WRITE(IOUNLOG,*) ' PBS', PBS(1),' ',  PBS(2) 
PBS = PBS / SUM(PBS)
!* WE PASS DATA FROM EAST TO WEST
WHERE(IBS.EQ.1 .OR. IBS.EQ.2) IBS = IBS+GRD%IM-2

END SUBROUTINE PREPINTERP3
