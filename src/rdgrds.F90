...
!-----------------------------------------------------------------------
  ...
  INTEGER(I4) :: JGRID, I, J, JLEV,KK, KSV,OCEANPOINTS
...
!  ALLOCATE GRID ARRAYS
    ...
    OCEANPOINTS=SUM(GRD%MSK)
    WRITE(IOUNLOG,*) ' OCEANPOINTS   :', OCEANPOINTS

    !MPI_ALLGATHER(OCEANPOINTS,1,MPI_INT,NPROCS,MPI_INT,MPI_COMM_WORLD,IERR)
    IF (OCEANPOINTS<10) THEN
        WRITE(IOUNLOG,*) "NO ENOUGH OCEAN POINTS IN THIS SPECIFIC DOMAIN,CHECK POINTS AND REDUCE NUMBER OF DOMAINS"
        CALL ABOR1('NO OCEAN POINTS IN ONE FF THE DOMAIN')
    ENDIF
