...
LOGICAL :: LL_SSH, LL_TQ2, LL_ICE
  ...
  IF(LMPION) THEN
     CALL CHECK( NF90_CREATE('ANINCR.NC'//'.'//TRIM(CMPIDOM), NF90_CLOBBER+NF90_64BIT_OFFSET, NCID) )
     !CALL CHECK( NF90_CREATE('ANINCR.NC'//'.'//TRIM(CMPIDOM), NF90_CLOBBER, NCID) )
  ELSE
     CALL CHECK( NF90_CREATE('ANINCR.NC', NF90_CLOBBER+NF90_64BIT_OFFSET, NCID) )
  ...
  !CALL CHECK( nf90_def_var_deflate(ncid,varID(4),0,1,1) )
  CALL CHECK( NF90_DEF_VAR(NCID, 'INCTEMPER',NF90_REAL, DIMIDS3, VARID(5)) )
  !CALL CHECK( nf90_def_var_deflate(ncid,varID(5),0,1,1) )
